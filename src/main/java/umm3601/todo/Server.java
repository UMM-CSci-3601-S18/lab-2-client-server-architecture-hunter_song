...
import umm3601.todo.UserController;
...
public class Server {
  ...
  public static final String USER_DATA_FILE = "src/main/data/todo.json";
  ...
  public static void main(String[] args) {
    ...
    // Get specific user
    get("api/todos/:id", userController::getUser);


    // List users, filtered using query parameters
    get("api/todos", userController::getUsers);

    // List a specified amount of users
    get("api/todos?limit=7", userController::getLimit);

    // List todos that are only complete(or incomplete)
    get("api/todos?status=complete", userController::getComplete);

    // List todos where the body contains a certain string
    get("api/todos", userController::getContains);
